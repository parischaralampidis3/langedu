...
class MultipleChoiceExerciseController extends Controller
{
    ...
    public function storeMcAnswer(Request $request, $id)
    {
        $McAnswer = $request->validate([
            'option_id' => 'required|exists:mc_options,id',
            'question_id' => 'required|exists:mc_questions,id'
        ]);
        $question = MultipleChoiceQuestion::findOrFail($id);

        $answer = new MultipleChoiceAnswer($McAnswer);

        $question->mc_answers()->save($answer);
